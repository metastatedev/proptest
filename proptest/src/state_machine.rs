...
use crate::std_facade::PhantomData;
use crate::std_facade::{Arc, Box, Cow, Vec};

use crate::collection::{self, SizeRange};
use crate::strategy::{BoxedStrategy, Strategy, Union, W};
...
impl<S: fmt::Debug + 'static> StateMachine<S> {
    ...
    /// the value.
    pub fn new(
        mutation_strategy: impl Strategy<Value = Box<dyn Mutator<S>>> + 'static,
        invariants: Arc<dyn Fn(&mut S) -> TestCaseResult>,
    ) -> Self {
        StateMachine {
    ...
    pub fn new_from_vecs(
        ...
        invariants: Vec<Box<dyn Fn(&mut S) -> TestCaseResult>>,
    ) -> Self {
        StateMachine::new(
    ...
    pub fn test_case(
        ...
        (state, collection::vec(self.mutation_strategy(), size)).prop_map(
            move |(state, mutations)| {
                StateMachineTestCase::new(clone.clone(), state, mutations)
            },
        )
    }
}
...
/// this.
pub struct DirectMutator<
    S,
    A: fmt::Debug,
    F: FnOnce(&mut S, A) -> TestCaseResult,
> {
    function: Option<F>,
    arguments: Option<A>,
...
impl<S, A: fmt::Debug, F: FnOnce(&mut S, A) -> TestCaseResult> fmt::Debug
    for DirectMutator<S, A, F>
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match (&self.arguments, &self.formatted_arguments) {
            (Some(ref args), _) => write!(f, "{}({:?})", self.name, args),
...
impl<S, A: fmt::Debug, F: FnOnce(&mut S, A) -> TestCaseResult> Mutator<S>
    for DirectMutator<S, A, F>
{
    fn mutate(&mut self, state: &mut S) -> TestCaseResult {
        let fun = self
            .function
            .take()
            .expect("Mutator applied more than once");
        let args = self
            .arguments
            .take()
            .expect("Mutator applied more than once");
    ...
    DirectMutator<S, A, F>
{
    ...
    pub fn no_debug(&mut self) {
        self.formatted_arguments =
            Some(Cow::Borrowed("<argument formatting disabled>"));
    }
}
